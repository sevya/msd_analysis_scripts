...
try:
	...
	path = "/dors/meilerlab/apps/scripts/libraries/python_library/"
...
class AnalysisTools():
	...
	def verbose(self, list_o):
		'''at.verbose returns nothing but prints to the screen whatever dictionary or list you called for analysis on
		...
		for key in list_o:
			...
			# sometimes it's not a dictionary, just a list of native residues
			# with their occurrences, there is another method for that
			else:
		...
		if bit is False:
			...
			for key in list_o:
				if type(self._arg_dict[key].values()[0]) == dict and key != 'n':
					self.bitscore_dict = self.seq_analysis.get_bit_scores(dict=self._arg_dict[key], normalize=normal)
					self.seq_analysis._write_out_dict(filename=key, prefix=prefix + "bit_", dict=self.bitscore_dict)
				else:
					if key == 'n':
						self.bitscore_dict = self.seq_analysis.get_bit_scores(dict=self.seq_analysis.analysis_dict, normalize=normal)
						bitscore_native = self.seq_analysis.get_native_residues(dict=self.bitscore_dict)
						self.seq_analysis._write_out_table(filename=key, prefix=prefix + "bit_", dict=bitscore_native,length_of_pdbs=1)
					if key == 'nd':
						self.bitscore_dict = self.seq_analysis.get_bit_scores(dict=self.seq_analysis.designed_dict, normalize=normal)
						self.bitscore_dict_native = self.seq_analysis.get_native_designed_residues(dict=self.bitscore_dict)
						self.seq_analysis._write_out_table(filename=key, prefix=prefix + "bit_", dict=self.bitscore_dict_native,length_of_pdbs=1)
	...
	def output_sequence_logos(self, output, native, sequence="", prefix="", format="", units="", stacks_per_line="", stack_width="",
		...
		command = seqlogos.initialize_options(format=format, units=units, stacks_per_line=stacks_per_line, stack_width=stack_width, title=title,
...
class SequenceLogos():
	...
	def __init__(self, pdbs, chain="", native=None, designed={}, sequence=True, output_prefix="Sequence_logo"):
		...
		if self.native:
			...
			elif sequence == "sequence_numbers":
				self.annotation = [ str(num[0])+str(num[1])+":"+seq for seq,num in zip( self.get_native_sequence_and_numbering(self.native,self.designed), sorted(self.designed))]

			else:
				self.annotation = self.get_native_sequence_and_numbering(
					self.native, self.designed, sequence=self.sequence)
		else:
			self.annotation = [str(i[0])+str(i[1]) for i in sorted(self.designed)]
	...
	def initialize_options(
		self, format="eps", sequence_type="protein", units="bits", first_index_number="1", lower_bound=None, upper_bound=None, size="large", stacks_per_line="40", stack_width="10.8", title="Sequence Logo", x_axis_label=None, y_axis_height="4.32", y_label="",
		...
		self.stack_width = ["-W", stack_width]
		self.title = ["-t", title]
		...
		self.combined_arguments = self.input_file + self.seq_output + self.format + self.sequence_style + self.units + self.first_index + self.size + self.title + self.annotate +\
			...
			self.fine_print + self.scale_width + self.composition + self.stacks + self.stack_width + self.y_label
