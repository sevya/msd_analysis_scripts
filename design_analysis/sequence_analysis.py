...
class DesignScoreTable():
	...
	def get_bit_scores(self,normalize=False,dict=""):
		...
		for line in dict:  # each line is a residue
			sum_l = 0.0  # or total number of amino acids at any given position, this is used for the frequency calculation
			p_entropy = 0.0  # entropy calculates how much a position is similar
			sum_l = sum([float(x) for x in dict[line].values()])  # add up all the amino acid occurrences, should equal the number of models or fastas you passed
			_return_dict[line] = dict[line]  # copy the key from the parameter dict
			for aa in dict[line]:  # iterate through the amino acids
				bit_score = 0.0
				frequency = float(dict[line][aa]) / float(sum_l)  # frequency is how many times you see that particular amino acid / how many amino acids total at that position
				try:
					bit_score = frequency*math.log(20.0*frequency, 2.0) 	 # frequency * log2(how many amino acids considered * frequency) #This is an approximation of the entropy explained in the WebLogo and Schneider 1990 papers
				except ValueError:
					bit_score = 0.0  # if we never saw an amino acid its bit score is zero
				_return_dict[line][aa] = float(("%.5f") % bit_score)  # bit score dictionary
			if normalize:
				p_entropy = sum(_return_dict[line].values())
				for aa in _return_dict[line]:
					_return_dict[line][aa] = ("%.5f") % ((_return_dict[line][aa]/p_entropy))  # normalize for each position by dividing it by the entropy (that is the maximum acheivable score)
			else:
				for aa in _return_dict[line]:
					# print aa, _return_dict[line][aa]
					_return_dict[line][aa] = ("%.5f") % (_return_dict[line][aa])  # else just give back the regular bit scores
		return _return_dict
		...
		with open(prefix+filename+"."+space, 'w') as f:
			...
			for key in sorted(dict):
				...
				for d in sorted(dict[key]):
					...
					f.write("{:^5.4f} ".format(occur).center(5))
				f.write("\n")
