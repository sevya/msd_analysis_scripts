...
if __name__ == '__main__':
	...
	parser.add_option("--native",dest="native",action='store_true',default=False, help='Only repack the residues on the designable side, don\'t \
								design them')

	parser.add_option("--repack",dest="repack",action='store_true',default=False, help='Repack the side of the interface that\'s not being designed')

	parser.add_option("--CB_dist_cutoff",dest="CB_dist_cutoff",
								help="Cutoff distance for Cbeta atoms to be considered part of the interface. \
	...
	# side1_list.sort(key=itemgetter(1,0))
	# side2_list.sort(key=itemgetter(1,0))
	if options.design=='1':
		#json.dump(side1_dict, open(options.output+".json", 'w'))
		pml_string = ''
		with open(options.output+'.resfile', 'w')  as out:
			out.write('NATRO EX 1 EX 2\nstart\n')
			...
			for key in side1_list:
				if options.native:
					out.write(key+' NATAA EX 1 EX 2\n')
				else:
					out.write(key+' ALLAA EX 1 EX 2\n')
			if options.repack:
